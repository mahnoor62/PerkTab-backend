...
async function createLevelConfig(payload = {}) {
  ...
  console.log(`[createLevelConfig] Saved level ${levelNumber} with ${processedColors.length} colors, ${processedDotSizes.length} dot sizes, and ${processedDots.length} dots`);
...
async function updateLevelConfig(levelNumber, payload) {
  ...
  if (Array.isArray(payload.dots)) {
    ...
    processedDots = validDots.map((dot, index) => {
...
async function updateLevelConfigById(levelId, payload) {
  ...
  if (payload.dots !== undefined && Array.isArray(payload.dots)) {
    ...
    if (validDots.length >= 0) { // Allow empty array to clear dots
      const processedDots = validDots.map((dot) => {
